        ...
        if play_message_client == "UTTT/1.0 406 FATAL_ERROR\n":
            ...
            if not position_client.isdigit() or len(position_client) != 2 or '9' in position_client:
                ...
                if etat == False:
                    ...
                    confirmation = connectionSocket.recv(1024).decode()
                    if confirmation != "UTTT/1.0 ACK\n":
                        connectionSocket.send("UTTT/1.0 406 FATAL_ERROR\n".encode())
                        connectionSocket.close()
                        break
                    else:
                        etat = True
                    ...
                    if time2 - time1 > time_out:
                        ...
                        if big_win(big_table_client)==True:
                            ...
                            while not response.startswith("UTTT/1.0 NEW_STATE"):
                                ...
                                clock.tick(60)
                                pygame.display.flip()
                                state_of_play_server_previous = construction_state_of_play(server_positions_array, client_positions_array)
                                position_server = str(xcord//3+(ycord//3)*3)+str(xcord%3+(ycord%3)*3)
                                play_message_server = f"UTTT/1.0 PLAY {position_server} {state_of_play_server_previous}\n"
                            ...
                            if etat == False:
                                ...
                                connectionSocket.send("UTTT/1.0 404 STATE_PLAY {position_server} {state_of_play_server_previous}\n".encode())
                                confirmation_client = connectionSocket.recv(1024).decode()
                                state_of_play_client = confirmation_client.split()[-1]
                                if state_of_play_client != state_of_play_server: 
                                    connectionSocket.send("UTTT/1.0 406 FATAL_ERROR\n".encode())
                                    connectionSocket.close()
                                    break
        ...
        for row in range(amtrow):
            ...
            for column in range(amtcol):
                if column % 3 == 0:
                    extramarginx = extramarginx + MARGIN
                if grid[column][row] == 1:
                    screen.blit(PNL, (((MARGIN + WIDTH) * column) + MARGIN + extramarginx,
                                                ((MARGIN + HEIGHT) * row) + MARGIN + extramarginy))
                elif grid[column][row] == 2:
                    screen.blit(PNL2, (((MARGIN + WIDTH) * column) + MARGIN + extramarginx,
                                                ((MARGIN + HEIGHT) * row) + MARGIN + extramarginy))
                else:
